...
export default function InviteModal({ leagueSlug, open, onClose }: InviteModalProps) {
  ...
  const generate = useCallback(async () => {
    ...
    setCopied(false)
    try {
      const data = await createInviteMutation.mutateAsync(leagueSlug)
      setInvite(data)
    } catch (err: any) {
      setError(err?.message || 'Erro ao gerar convite')
    }
  }, [leagueSlug, createInviteMutation])
  ...
  const copyUrl = useCallback(async () => {
    const url = `${window.location.origin}/convite/${invite?.code ?? ''}`
    try {
  ...
  return (
    <Modal open={open} onClose={onClose} className="w-full max-w-md">
      ...
      <FadeIn show={!invite && !createInviteMutation.isPending && !error}>
          ...
          <div>
            <div className="px-6 pb-6">
              <div className="bg-gradient-to-b from-green-cover-bg to-green-cover-btn rounded-2xl p-6 text-center relative overflow-hidden">
                ...
                <p ref={codeRef} className="text-4xl font-black text-green tracking-[0.25em] select-all relative mb-3">
                  {invite.code}
                </p>
            ...
            <div className="px-6 py-5">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={`${window.location.origin}/convite/${invite.code}`}
                  className="flex-1 px-3 py-2.5 rounded-lg border border-line text-xs text-gray-400 bg-gray-50 outline-none select-all"
                  onFocus={(e) => e.target.select()}
                />
                <button onClick={copyUrl} className="px-4 py-2.5 rounded-lg border border-line text-xs font-semibold text-gray-500 hover:bg-gray-50 transition-colors whitespace-nowrap cursor-pointer">
                  Copiar link
                </button>
              </div>
            </div>
            ...
            <div className="px-6 pb-6">
              <button onClick={generate} className="w-full py-2.5 rounded-lg border border-dashed border-gray-300 text-sm font-semibold text-gray-400 hover:text-gray-500 hover:border-gray-400 hover:bg-gray-50 transition-colors cursor-pointer">
                Gerar novo código
              </button>
            </div>
          </div>
