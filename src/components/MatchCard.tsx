...
export default function MatchCard({ match, guess, onGuessChange, saving, blocked }: MatchCardProps) {
  const matchDate = new Date(match.date)
  const isInvalidDate = !match.date
  ...
  const { isFinished, isLive, isLocked, isPreMatchLocked, isUnscheduled, isOpen } = useMemo(() => {
    ...
    if (isTbd) {
      return { isFinished: false, isLive: false, isLocked: true, isPreMatchLocked: false, isUnscheduled: false }
    }
    const now = Date.now()
    ...
    return {
      ...
      isLive: !match.finished && (match.locked || startsIn <= 0),
  ...
  return (
    <div
      className={`bg-white rounded-lg border p-4 transition-all hover:shadow-normal ${
        isUnscheduled || isTbd ? 'border-dashed border-gray-200'
        : isLive ? 'border-line border-l-4 border-l-red-widget'
        : isPreMatchLocked ? 'border-line border-l-4 border-l-yellow-400'
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-gray-300 font-medium uppercase tracking-wide">
          {translatePhase(match.phase) && `${translatePhase(match.phase)}${match.round ? ` - ${match.round}` : ''}`}
        </span>
        <div className="flex items-center gap-2">
          ...
          {isUnscheduled && (
            <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-sm uppercase">
              <svg width="10" height="10" viewBox="0 0 12 12" fill="none">
                ...
                <path d="M4 0.5V3.5M8 0.5V3.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
          ...
          {isPreMatchLocked && (
            <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-sm uppercase">
              <svg width="10" height="10" viewBox="0 0 12 12" fill="none">
                ...
                <path d="M4 5.5V3.5C4 2.4 4.8 1.5 6 1.5C7.2 1.5 8 2.4 8 3.5V5.5" stroke="currentColor" strokeWidth="1.2" />
              </svg>
              FECHADO
            </span>
          )}
          ...
          <span className="text-xs text-gray-300">
            {isUnscheduled ? 'Data a definir'
            : isLive ? 'Agora'
            : isPreMatchLocked ? format(matchDate, "HH:mm", { locale: ptBR })
          ...
          <div className="flex flex-col items-center gap-1 sm:gap-1.5">
            ...
            {guess?.score !== undefined && (
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${
                guess.score === 10 ? 'bg-green-cover-bg'
                : guess.score > 0 ? 'bg-amber-50'
                : 'bg-red-50'
              }`}>
                <span className="text-[10px] font-semibold text-gray-300 uppercase tracking-wide">Seu palpite</span>
                <span className={`text-sm font-bold ${
                  guess.score === 10 ? 'text-green'
                  : guess.score > 0 ? 'text-amber-700'
                  : 'text-red-widget'
                }`}>
                ...
                <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${
                  guess.score === 10 ? 'bg-green text-white'
                  : guess.score > 0 ? 'bg-amber-200 text-amber-800'
                  : 'bg-red-200 text-red-800'
                }`}>
                  {guess.score === 10 ? 'Exato +10'
